...
clear FUELS
% HTPB - AP-80% Fe2O3-3%
FUELS(1) = RocketFuel("HTPB/AP[75%]/Fe2O3[3%]", 1854.55362, 3440, 1.25, 320, 0.323, 10.31*10^(-(3+6*0.323)));
% HTPB - AP-80%
FUELS(2) = RocketFuel("HTPB/AP[70%]", 1854.55362, 3440, 1.25, 320, 0.098, 6.2*10^(-(3+6*0.098)));
...
for i = 1:length(AeAt)
    ...
    for j = 1:length(FUELS)
        params.fuel = FUELS(j);
        % Calculate P0
        P0 = (params.fuel.TCoefficient * params.fuel.Density * params.fuel.cv * Ab / At)^(1 / (1 - params.fuel.Stability));
        % Mass flow rate
...
for k = 1:length(results)
    if size(results{k}) > 0
        legendstrings{end+1} = FUELS(k).Name;
...
for k = 1:length(results)
    if size(results{k}) > 0
        legendstrings{end+1} = num2str((Ab - pi*params.r_max^2)/(2*pi*params.r_max),2) + "m";
...
for j = 1:length(FUELS)
    params.fuel = FUELS(j);
    P0 = (params.fuel.getCV()*params.fuel.Density*params.fuel.TCoefficient*Ab(j)/At)^(1/(1-params.fuel.Stability));
    results{1} = [results{1}, [(Ab - pi*params.r_max^2)/(2*pi*params.r_max); P0]];
    %fprintf('\nFound Solution @ AeAt = %.2f, Ab = %.6f\n', AeAt(i), Ab(j));
end
